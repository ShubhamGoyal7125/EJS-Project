...
app.post("/compose", function (req, res) {
  const post = {
    inputText: req.body.composeText,
    inputTitle: req.body.composeTitle
  };
  posts.push(post);
  res.redirect("/");
});
...
app.get("/posts/:topic", function (req, res){
  const postTopic = lodash.lowerCase(req.params.topic);
  posts.forEach(item => {
    const itemTitle = lodash.lowerCase(item.inputTitle);
    if(itemTitle === postTopic){
      res.render("post", {title: item.inputTitle, text: item.inputText});
    }
  });
});
